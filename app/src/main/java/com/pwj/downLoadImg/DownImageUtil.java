...
public class DownImageUtil {
    ...
    public void setImageCallBacks(ImageCallBacks imageCallBacks) {
        mImageCallBacks = imageCallBacks;
    }
    /**
     * 执行单线程列队执行
     */
                    ...
                    public void onDownLoadSuccess(File file) {
                       final String desPath = DownFileUtils.getImagesDir(mContext, sort) + i + ".png";
                        ...
                        ((Activity)mContext).runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                mImageCallBack.onSuccess(i,desPath);
                            }
                        });
    ...
    public void onDownLoads(String url, final int i,final int j,final String path,final boolean flag) {
        DownLoadImageService service = new DownLoadImageService(mContext,
                url,
                new ImageDownLoadCallBack() {
                    @Override
                    public void onDownLoadSuccess(File file) {
                        final String desPath = DownFileUtils.getImagesDir(mContext, "pdf") + path;
                        savePhotoToSDCard(file.getPath(), desPath);
                        if (flag){
                            ((Activity)mContext).runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    mImageCallBacks.onSuccess(i,j,desPath);
                                }
                            });
                        }
                    }
                    ...
                    @Override
                    public void onDownLoadFailed() {
                        mImageCallBacks.onFailed();
                    }
                });
        //启动图片下载线程
        runOnQueue(service);
    }
//    public void onDownLoads(String url, final int i,final int j,int number, final String sort) {
//        DownLoadImageService service = new DownLoadImageService(mContext,
//                url,
//                new ImageDownLoadCallBack() {
//                    @Override
//                    public void onDownLoadSuccess(File file) {
//                        final String desPath = DownFileUtils.getImagesDir(mContext, sort) + number + ".png";
//                        savePhotoToSDCard(file.getPath(), desPath);
//                        ((Activity)mContext).runOnUiThread(new Runnable() {
//                            @Override
//                            public void run() {
