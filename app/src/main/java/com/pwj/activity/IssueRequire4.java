...
public class IssueRequire4 extends BaseActivityIssueRequire {
...
//    private EditText et3;
    ...
    public void initChild() {
        initLayout(R.layout.activity_require4);
        initView();
    }

    private void initView() {
        title_tv.setText("发布物流需求");
...
//            }
    ...
    public void insert1() {
        for (int i = 0; i < mUrls.length; i++) {
            ...
            EasyHttp.post(IpConfig.URL_IMG + names)
                    ...
                    .timeStamp(true)
                    .execute(new ProgressDialogCallBack<String>(mProgressDialog, true, false) {
                        @Override
    ...
    private void insert2() {
        EasyHttp.post(IpConfig.URL_SQL)
                .params("query", "insert into " + ProductTableName.require4 + "(`url`,`url_type`,`contact`,`phone`,`phone_address`,`location`,`province`,`city`,`county`,`street`,`specific`,`name`,`destination`, `remarks`, `content`, `date`)values('" + url + "','" + url_type + "','" + contact + "','" + phone + "','" + phone_address + "','" + location + "','" + province + "','" + city + "','" + county + "','" + street + "','" + specific + "','" + et1_str + "','" + et2_str + "','" + remarks + "','" + content + "','" + date + "')")
                .timeStamp(true)
                .execute(new SuccessCallBack<String>() {
                    @Override
                    public void onSuccess(String s) {
                        insertIssue(phone, ProductTableName.require4);
                    }
                });
    }
