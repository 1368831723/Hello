...
public class IssueProductType3 extends BaseActivityIssue {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        initLayout(R.layout.activity_product_3_casting);
        initView();
    }
...
//            } else {
            ...
            if (!TextUtils.isEmpty(et2_str)) {
                content = content + "规格：" + et2_str + "。\n";
            }
            if (!TextUtils.isEmpty(et3_str)) {
                content = content + "单价：" + et3_str + "。\n";
            }
        ...
        for (int i = 0; i < mUrls.length; i++) {
            ...
            if (i == mUrls.length - 1) {
                ...
                EasyHttp.post(IpConfig.URL_SQL)
                        .params("query", "insert into " + table_product[type] + "(`url`,`url_type`,`contact`,`phone`,`phone_address`,`consign`,`postage`,`location`,`product_name`,`spec`,`price`,`description`,`content`,`date`,`province`,`city`,`county`,`street`,`specific`)values('" + url + "','" + url_type + "','" + contact + "','" + phone + "','" + phone_address + "','" + consign + "','" + postage + "','" + location + "','" + et1_str + "','" + et2_str + "','" + et3_str + "','" + description + "','" + content + "','" + date + "','" + province + "','" + city + "','" + county + "','" + street + "','" + specific + "')")
                        .timeStamp(true)
