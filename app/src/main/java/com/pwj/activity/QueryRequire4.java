...
public class QueryRequire4 extends BaseActivityQueryRequire {
    private MyAdapter myAdapter;

    @Override
    public void initData() {
        data = new ArrayList<>();
        title_tv.setText("物流需求");
        queryType();
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.issue:
                startActivity(IssueRequire4.class);
                finish();
                break;
        }
    }
    ...
    public void queryType() {
        EasyHttp.post(IpConfig.URL_SQL)
                .params("query","SELECT url,name,location,destination,remarks,contact,phone FROM logistics order by id desc")
                .timeStamp(true)
                                    ...
                                    .execute(new SuccessCallBack<String>() {
                                        ...
                                        public void onSuccess(String s) {
                                            ...
                                            if (data_users.size()==data.size()){
                                                progressbar.setVisibility(View.GONE);
                                                recycler.setLayoutManager(new LinearLayoutManager(QueryRequire4.this, LinearLayoutManager.VERTICAL, false));
    ...
    private class MyAdapter extends RecyclerView.Adapter<MyAdapter.MyViewHolder> {
        ...
        public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(QueryRequire4.this).inflate(R.layout.item_product, parent, false);
        ...
        public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            ...
            holder.gridView.setAdapter(new AssNineGridViewClickAdapter(QueryRequire4.this, imageInfos));
            url = data_users.get(position).getUrl3();
            String user_name = data_users.get(position).getUser_name();
            String tv1 = data.get(position).getName()  + ","+data.get(position).getLocation() + "," +data.get(position).getDestination() + ","  ;
            String tv2 = data.get(position).getContact() + "," + data.get(position).getPhone()+ "," + data.get(position).getRemarks() ;
            holder.tv.setText(tv1 + tv2);
            Glide.with(QueryRequire4.this).load(url).into(holder.img);
            holder.name_tv.setText(user_name);
            holder.img.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(QueryRequire4.this, ChatActivity.class);
