...
public class IssueRequireOthers extends BaseActivityIssueRequire {
    ...
    public void initChild() {
        initLayout(R.layout.activity_require0);
        initView();
    }

    private void initView() {
        title_tv.setText("发布其他需求");
...
//                Util.showToast(this, "需求类型不能为空");
//            } else {
//            content = "需求名称：" + et1_str + "。\n位置：" + location + "。\n姓名：" + contact + "。\n电话：" + phone_address + "。" + remarks;
...
//            }
    ...
    private void insert2() {
        EasyHttp.post(IpConfig.URL_SQL)
                .params("query", "insert into " + ProductTableName.require0 + "(`url`,`url_type`,`contact`,`phone`,`phone_address`,`location`,`province`,`city`,`county`,`street`,`specific`,`name`, `remarks`, `content`, `date`)values('" + url + "','" + url_type + "','" + contact + "','" + phone + "','" + phone_address + "','" + location + "','" + province + "','" + city + "','" + county + "','" + street + "','" + specific + "','" + et1_str + "','" + remarks + "','" + content + "','" + date + "')")
                .timeStamp(true)
                .execute(new SuccessCallBack<String>() {
                    @Override
                    public void onSuccess(String s) {
                        insertIssue(phone, ProductTableName.require0);
                    }
                });
    }
