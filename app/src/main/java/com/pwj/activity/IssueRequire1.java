...
public class IssueRequire1 extends BaseActivityIssueRequire {
    ...
    public void initChild() {
        initLayout(R.layout.activity_require1);
        initView();
    }

    private void initView() {
        title_tv.setText("发布求购需求");
    ...
    private void insert2() {
        EasyHttp.post(IpConfig.URL_SQL)
                .params("query", "insert into " + ProductTableName.require1 + "(`url`,`url_type`,`contact`,`phone`,`phone_address`,`location`,`province`,`city`,`county`,`street`,`specific`,`name`, `remarks`, `content`, `date`)values('" + url + "','" + url_type + "','" + contact + "','" + phone + "','" + phone_address + "','" + location + "','" + province + "','" + city + "','" + county + "','" + street + "','" + specific + "','" + et1_str + "','" + remarks + "','" + content + "','" + date + "')")
                .timeStamp(true)
                .execute(new SuccessCallBack<String>() {
                    @Override
                    public void onSuccess(String s) {
                        insertIssue(phone, ProductTableName.require1);
                    }
                });
    }
